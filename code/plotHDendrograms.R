...
gs.dir = file.path(data.dir,'out/hierarchical_geneScore/')

args<-list(
  #score_file=file.path(gs.dir,'0.1cM_chr22.pmi_full.tab'),
  score_file=file.path(gs.dir,'RA_OKADA_IMB.pmi_full.tab'),
  sets = file.path(data.dir,'support/javierre_tree.yaml'),
  geneName = 'AHR'
)
...
merged.f<-subset(merged,overall_gene_score>all.thresh & biotype=='protein_coding')
...
Nodelabel <- function(node) {
  if(node$isLeaf)
    return(paste0( node$name,': GS ', format(signif(node$ppi,digits=2), scientific = ifelse(node$ppi<0.001,TRUE,FALSE), big.mark = "'")))
  if(node$above.thresh)
    return(paste0( node$name,'\n pBF ', ifelse(is.na(node$BF),'NA',format(signif(node$BF,digits=2), scientific = ifelse(node$BF>100,TRUE,FALSE), big.mark = "'"))))
  return(paste0(node$name,'\n'))
}
...
pdf("~/gitr/fyr/pics/AHR_dend.pdf")
par(mar=c(1,1,1,1))
...
plot(gnp, show.tip.label = TRUE, type = "cladogram",edge.color="grey",edge.width=0.5, plot=FALSE)
...
pdf("~/gitr/fyr/pics/RA_dend.pdf")
par(mar=c(1,1,1,1))
plot(wnp, show.tip.label = TRUE, type = "cladogram",edge.color="grey",edge.width=0.5,plot=FALSE,main="")
